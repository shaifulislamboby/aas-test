...
def create_base64_form(submodelIdentifier: str) -> base64:
...
class AASEndPoints:
    ...
    def set_submodel_identifier(self) -> None:
        ...
        if not error:
            response = create_base64_form(response)
    ...
    def set_cd_identifier(self):
        if self.full_url_path.startswith('/concept'):
            try:
                ...
                _response = self.parse_response(response=response)
                self.cd_identifier = create_base64_form(_response.get('identification').get('id'))
            except Exception as error:
    ...
    def set_package_identifier(self):
        if self.full_url_path.startswith('/packages'):
            try:
                ...
                _response = self.parse_response(response=response)
                self.package_identifier = create_base64_form(_response.get('identification').get('id'))
            except Exception as error:
                # need to add things to log or file
                pass
...
class AasSchemaParser:
    ...
    def raw_schema(self) -> dict:
        return json.loads(self.file_location)
