...
class PreparePPDPositive(Preparation):
    ...
    def set_all_required_attributes(self, positive=True):
        if self.is_implemented and "get" in self.operations:
            ...
            if self.is_implemented:
                ...
                self.single_get_response = self.get_single_object_from_response(
    ...
    def set_all_responses(self):
        ...
        if self.is_implemented:
            if self.post_data:
                self.post_response = (
                    self.session.post(url, json=self.post_data)
                    if self.session
                    else requests.post(url, json=self.post_data)
                )
                self.create_query_params(operation="post")
                if self.post_query_params:
                    self.create_response_with_query_params(
                        operation="post", url=url, json_data=self.post_data
                    )
            if self.put_data:
                self.put_response = (
                    self.session.put(url, json=self.put_data)
                    if self.session
                    else requests.put(url, json=self.put_data)
                )
                self.create_query_params(operation="put")
                if self.put_query_params:
                    self.create_response_with_query_params(
                        operation="put", url=url, json_data=self.put_data
                    )

    @aas_logger
    def create_response_with_query_params(
        self, operation, url, json_data: Union[None, dict] = None
    ) -> None:
        ...
        if _attr:
            for param in _attr:
                ...
                if operation in ("post", "put"):
                    res = (
                        method(url=url_, json=json_data)
                        if not self.session
                        else session_(url=url_, json=json_data)
                    )
                    setattr(self, f"{operation}_{param}_response", res)
                else:
        ...
        if query_params:
            ...
            if list_of_query_keys:
                ...
                if len(list_of_query_keys) > 1:
                    query_combinations = self.create_list_combinations(
                        list_of_query_keys
                    )
                    for combination in query_combinations:
                        dict_combinations: list[dict] = [
                            self.query_params.get(operation).get(value)
    ...
    def create_list_combinations(value: list) -> list[list]:
        ...
        """
        initial = sum(
            [list(map(list, combinations(value, i))) for i in range(len(value) + 1)], []
        )
