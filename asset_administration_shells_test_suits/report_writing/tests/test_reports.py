...
class TestReports(unittest.TestCase):
    ...
    def test_write_test_results_to_file(self) -> None:
        ...
        with open('abc.txt', 'w') as file:
            ...
            for uri, operation, f, length in test_values:
                for test_result in (self.test_result, 'not implemented', 111, self.test_result_passed):
                    with self.subTest(f'testing for value:{uri, operation, f}'):
                        r = write_test_results_to_file(test_result, uri, operation, self.base_preparation, f)
                        l_length.append(r)
    ...
    def test_write_non_implemented_test_results_to_file(self) -> None:
        with open('abc.txt', 'w') as file_one:
            test_values = (('abc', 'get', file_one, 130), ('bcd', 'post', file_one, 133))
            for uri, operation, f, length in test_values:
                for test_result in (self.test_result, 'not implemented', 111, self.test_result_passed):
                    with self.subTest(f'testing for value:{uri, operation, f}'):
                        ...
                        self.assertGreater(r, length, 'this written message is greater then the normal message')
